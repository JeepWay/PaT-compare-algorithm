...
def train(config: Dict[str, Any]):
    ...
    model = DDQN(env=vec_env, **config["DDQN_kwargs"], tensorboard_log=config['save_path'])
    with open(os.path.join(config['save_path'], "model.txt"), "w") as f:
        f.write(f"DDQN's network architecture: \n{str(model.policy)}\n")
        f.write(f"\nDDQN's number of parameters: {sum(p.numel() for p in model.policy.parameters())}\n")
    model.learn(
...
def test(config: Dict[str, Any]):
    ...
    for i in range(5):
        ...
        model = DDQN.load(os.path.join(config['test_dir'], config["env_id"]), **config["DDQN_kwargs"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_path', default="settings/v1_deeppack_DDQN-h200-rC.yaml", type=str, help="Path to the configuration file with .yaml extension.")
