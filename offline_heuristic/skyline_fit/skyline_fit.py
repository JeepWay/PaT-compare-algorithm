import numpy as np
from skyline_fit.common.constants import BIN, MASK
import json
import logging
...
class SkylineBottomLeft:
    ...
    def predict(self, observation, state=None, episode_start=None, deterministic=True):
        obs_bin = observation[BIN]
        if obs_bin.ndim == 3:
            # case: shape (3, 10, 10)
            bin_data = obs_bin[0]
            width = int(obs_bin[1, 0, 0])
            height = int(obs_bin[2, 0, 0])
        elif obs_bin.ndim == 4:
            # case: shape (batch_size, 3, 10, 10)
            bin_data = obs_bin[0][0]
            width = int(obs_bin[0, 1, 0, 0])
            height = int(obs_bin[0, 2, 0, 0])
        else:
            raise ValueError(f"Unsupported observation shape: {obs_bin.shape}")
